...
std::vector<std::vector<std::string>> group_hosts_into_meshes(
    ...
    std::vector<std::vector<std::string>> meshes;
    ...
    for (const auto& corner : corners) {
        ...
        while (!new_corners.empty()) {
            ...
            if (result.success) {
                ...
                for (const auto& [rank, host] : result.target_to_global) {
                    mesh.push_back(host);
                    visited_hosts.insert(host);
                }
                meshes.push_back(mesh);
                auto [pruned_host_adjacency, potential_corners] = prune_host_adjacency(current_host_adjacency, mesh);
