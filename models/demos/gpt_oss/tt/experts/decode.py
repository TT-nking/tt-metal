...
def decode_forward(
    ...
    gate = ttnn.sparse_matmul(
        ...
        program_config=program_config.get_decode_gate_up_config(
            hidden_states.shape[2], weights.gate_proj.shape[3], hidden_states.shape[2]
        ),
    ...
    up = ttnn.sparse_matmul(
        ...
        output_tile=output_tile,
        program_config=program_config.get_decode_gate_up_config(
            hidden_states.shape[2], weights.up_proj.shape[3], hidden_states.shape[2]
        ),
        dtype=activation_dtype,
    )
    ...
    down = ttnn.sparse_matmul(
        ...
        program_config=program_config.get_decode_down_config(
            down_input.shape[2], weights.down_proj.shape[-1], down_input.shape[3]
        ),
        dtype=activation_dtype,
    )
