...
@pytest.mark.parametrize("alpha_p, alpha_n", [(0.5, 1.0), (1.0, 0.5)])
def test_xielu(alpha_p, alpha_n, device):
    """Test ttnn.xielu (xIELU - Expanded Integral of the Exponential Linear Unit) with FLOAT32."""
    torch.manual_seed(42)
    torch_input = torch.rand([4, 4], dtype=torch.float32)
    golden_fn = ttnn.get_golden_function(ttnn.xielu)
    torch_output = golden_fn(torch_input, alpha_p, alpha_n)
...
def test_xielu_golden(device):
    """Test ttnn.xielu against golden reference (like test_pow.py lines 75-76)."""
