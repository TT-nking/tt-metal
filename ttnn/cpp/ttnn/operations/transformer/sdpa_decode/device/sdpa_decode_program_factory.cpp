...
SdpaDecodeProgramFactory::cached_program_t SdpaDecodeProgramFactory::create(
    ...
    // ========== Spatial Indexing Mode ==========
    // Column-major group indexing: enabled when q_parallel_factor > 1 (Q heads is parallelized)
    // This keeps batch groups spatially close for efficient K multicast along columns
    const bool use_col_major_group_indexing =
        (q_heads_parallel_factor > 1) && (grid_size.y >= num_cores_per_head) && !on_subcoregrid;
    ...
    // Input CBs
    create_cb(CBIndex::c_0, q_tiles * q_tile_size, q_df, q_tile_size, &q_tile);               // Q input
    create_cb(CBIndex::c_1, k_tiles * k_tile_size, k_df, k_tile_size, &k_tile);               // K input
    create_cb(CBIndex::c_2, v_tiles * v_tile_size, v_df, v_tile_size, &v_tile);               // V input
    ...
    for (uint32_t i = 0; i < num_active_cores; ++i) {
        ...
        if (use_col_major_group_indexing) {
            ...
            cur_head = (num_cores_per_head == 0) ? 0 : (i % num_cores_per_batch) / num_cores_per_head;
            core_num_in_reduce = (num_cores_per_head == 0) ? 0 : i % num_cores_per_head;
        ...
        // For column-major indexing with horizontal reduction groups:
...
void SdpaDecodeProgramFactory::override_runtime_arguments(
    ...
    for (uint32_t i = 0; i < num_active_cores; ++i) {
        ...
        // Core role identification - same logic as in create()
        // Horizontal reduction groups: 4 consecutive x values form a group
        // Column-major batch indexing: batch = group_col * grid_size.y + core.y
        uint32_t cur_batch, cur_head, core_num_in_reduce, core_num_in_output;
        if (use_col_major_group_indexing) {
            uint32_t group_col = core.x / num_cores_per_head;  // which batch group column
            cur_batch = group_col * grid_size.y + core.y;      // column-major batch indexing
            cur_head = 0;
            core_num_in_reduce = core.x % num_cores_per_head;  // position within horizontal reduction group
            core_num_in_output = core_num_in_reduce;
        } else {
            cur_batch = i / num_cores_per_batch;
            cur_head = (num_cores_per_head == 0) ? 0 : (i % num_cores_per_batch) / num_cores_per_head;
            core_num_in_reduce = (num_cores_per_head == 0) ? 0 : i % num_cores_per_head;
