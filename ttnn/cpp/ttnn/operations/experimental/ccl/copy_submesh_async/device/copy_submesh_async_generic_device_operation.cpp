...
void CopySubmeshAsyncGenericDeviceOperation::validate_on_program_cache_miss(
    ...
    TT_FATAL(
        ...
        "CopySubmeshAsync: input tensor dimension {} must be divisible by num_devices {}",
...
void CopySubmeshAsyncGenericDeviceOperation::validate_on_program_cache_hit(
    ...
    if (persistent_output_buffer.has_value()) {
        ...
        TT_FATAL(
            ...
            "Output tensor for copy_submesh_async must be on device");
        TT_FATAL(
            output_tensor.buffer()->buffer_type() == BufferType::DRAM,
            "Output tensor for copy_submesh_async must be in DRAM, but is in {}",
        ...
        TT_FATAL(
            ...
            "Output tensor shape {} must match expected output tensor shape {} for CopySubmeshAsync",
            output_shape,
...
Tensor copy_submesh_async_generic(
    ...
    TT_FATAL(
        num_devices > 1,
        "copy_submesh_async is a collective operation and requires more than 1 device, but has {}",
        num_devices);
